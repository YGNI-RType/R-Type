...
namespace ecs {
    ...
    private:
        ComponentSparseArray<Transform>& transforms;
        ComponentSparseArray<Motion>& velocities;
    };

    class DrawSystem : public System {
    public:
        DrawSystem(Core &core)
            : sprites(core.getComponents<Sprite>()), transforms(core.getComponents<Transform>()), wd(sf::VideoMode(1000, 800), "title")
        {
        }
    ...
    private:
        float m_time = 0;
        sf::RenderWindow wd;
        ComponentSparseArray<Sprite>& sprites;
        ComponentSparseArray<Transform>& transforms;
    };
    ...
    public:
        KillerSystem(Core &core)
            : transforms(core.getComponents<Transform>()), m_core(core)
        {
        }
