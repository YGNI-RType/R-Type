...
public:
    ...
    void subscribeToEvent(void (Derived::*callbackMethod)(EventType &)) {
        m_eventBus->get().template subscribe<Derived, EventType>(
            static_cast<Derived &>(*this), callbackMethod);
    }

    template <typename T> T &getSystem(void) {
        static_assert(is_one_of<T, DependTypes...>::value,
                      "SystemType is not in the list of allowed types");
        return m_ecs->get().template getSystem<T>();
    }

    template <typename T> component::SparseArray<T> &getComponent(void) {
        static_assert(is_one_of<T, DependTypes...>::value,
                      "ComponentType is not in the list of allowed types");
