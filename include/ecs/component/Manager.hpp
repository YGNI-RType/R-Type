...
public:
    ...
    template <class Component> SparseArray<Component> &registerComponent() {
        auto res = m_componentArrays.emplace(
            ...
            std::make_pair(SparseArray<Component>(),
    ...
    void setComponent(entity::Entity entity, Params &&...p) {
        getComponents<Component>().emplace(
            entity, Component(std::forward<Params>(p)...));
    }
    ...
    template <class Component> SparseArray<Component> &getComponents() {
        ...
        if (it == m_componentArrays.end())
            throw std::runtime_error("The component " +
...
private:
    ...
    std::unordered_map<std::type_index, std::pair<std::any, destroyer_t>>
        m_componentArrays;
};
